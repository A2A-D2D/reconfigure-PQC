...
rtl/
  ...
  falcon_fft_addr_gen.v             # Falcon FFT/iFFT address + GM index generator
  falcon_fft_stage_ctrl.v           # full stage/batch scheduler
  falcon_fft_batch_exu.v            # 5-lane shared/pipe FE backend wrapper
  falcon_fft_task_engine.v          # load/execute/store task engine shell
  falcon_fft_local_buffer.v         # two-bank local f64 scalar buffer
  falcon_fft_twiddle_cache.v        # independent GM twiddle cache
  falcon_fft_buffered_engine.v      # local-buffered performance path
```
...
spuv3_vpu_fe_wrap
    ...
    +-- falcon_fft_stage_ctrl / falcon_fft_addr_gen
    +-- VR/DLM read packer
    +-- falcon_fft_local_buffer
    +-- falcon_fft_twiddle_cache
    +-- falcon_fft_batch_exu / reconfig_fft_f64_*_operator
...
1. 保留当前 FE RTL，新增 `spuv3_vpu_fe_wrap`，先用 SFR/CSR command 方式接入控制面。
2. 数据面以 256-bit DLM/DPRAM、320-bit VR/VPU 和 FE local buffer 为主，不把 CSR 当作 Falcon FFT 数据存储。
3. 实现 VR/DLM 到 FE 输入向量的 pack/unpack，优先支持 `LANES=5` 与 320-bit VR 单拍对齐。
4. 保留 shared backend 作为面积优先配置，同时使用 pipelined backend 作为性能优先配置。
5. 完善 `falcon_fft_buffered_engine`，让 stage loop、地址生成、twiddle 读取和本地 buffer ping-pong 在 FE 内部连续运行。
6. 完善 twiddle cache 和 FFT permutation helper，包括特殊 twiddle bypass。
7. 把 command 方式收敛为正式 XPqc VFE 指令编码，并接入 LLVM/assembler。
8. 将 `falcon_fft_buffered_engine` 接到 SPUV3 VPU/XPqc task scheduler。
9. 用真实 DLM/DPRAM preload/drain 路径验证数值正确性和吞吐。
