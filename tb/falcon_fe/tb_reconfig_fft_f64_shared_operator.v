...
module tb_reconfig_fft_f64_shared_operator;
    localparam LANES = 2;
    ...
    task start_and_wait;
        ...
        begin
            ...
            wait_count = 0;
            while (!valid_out && wait_count < 20) begin
                @(posedge clk);
                #1;
                wait_count = wait_count + 1;
            end
            if (!valid_out) begin
                $display("TB_FAIL shared operator timeout waiting valid_out");
                errors = errors + 1;
            end
        end
    endtask
    ...
    task check_ct;
        begin
            ...
            check_word("ct_l1_va_re", va_out_re_vec[1*64 +: 64], F64_100);
            check_word("ct_l1_va_im", va_out_im_vec[1*64 +: 64], F64_050);
            check_word("ct_l1_vb_re", vb_out_re_vec[1*64 +: 64], F64_ZERO);
    ...
    task check_gs;
        begin
            ...
            check_word("gs_l1_va_re", va_out_re_vec[1*64 +: 64], F64_100);
            check_word("gs_l1_va_im", va_out_im_vec[1*64 +: 64], F64_ZERO);
            check_word("gs_l1_vb_re", vb_out_re_vec[1*64 +: 64], F64_ZERO);
    ...
    initial begin
        ...
        if (errors == 0) begin
            $display("TB_PASS all shared f64 FFT operator cases");
        end else begin
